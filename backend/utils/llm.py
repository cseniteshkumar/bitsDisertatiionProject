"""LLM utilities for the backend package.

This file contains the concrete implementation of the central
`create_chat_completion` helper and related LLM utilities. During the
previous consolidation the canonical implementation was copied here as a
safe local fallback so other modules may import `create_chat_completion`
from `backend.utils.llm` directly.
"""
...
from backend.llm_provider.generic.base import (
    NO_SUPPORT_TEMPERATURE_MODELS,
    SUPPORT_REASONING_EFFORT_MODELS,
    ReasoningEfforts,
)

from backend.prompts import PromptFamily
from .costs import calculate_llm_cost
from .validators import Subtopics


def get_llm(llm_provider: str, **kwargs):
    from backend.llm_provider import GenericLLMProvider
